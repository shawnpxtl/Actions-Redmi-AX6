...
## 针对AX6的说明(LEDE\OpenWRT分支刷入方式通用)
#### 注意：千万不要在OpenWRT分支下直接升级LEDE固件，必须使用下面的方法刷入，反之亦然，否则因作死成功出现的各种问题本人概不负责，可能还会取笑你233

1. 上传自己的`AX6.config\AX6-LEDE.config`文件(看你要编译哪个分支)
2. 进入Actions页面手动启动编译
3. 当编译完成后，在Releases页面(你要刷OpenWRT就选AX6-OpenWRT的，反之则为AX6-LEDE)下载含编译出的`xxx-factory.ubi`固件(如果要在op下升级则是下载`xxx-sysupgrade.bin`后缀的固件)
